...
var DedicatedPages = (() => {
  ...
  const PAGES = {
    ...
    // 🆕 NEW (Apr 2026): One-stop settings page replacing the old in-tab UI.
    'all-settings': {
      title: 'الإعدادات',
      icon: '⚙️',
      heading: 'كل الإعدادات في مكان واحد',
      description: 'كل ما تحتاجه لإدارة حسابك وتفضيلاتك',
      render: renderAllSettingsPage
    }
  };
  ...
  // ═══ ALL SETTINGS — single dedicated page (Apr 2026) ═══════════════
  // Replaces the old in-tab settings layout.
  // - Sidebar → "كل الإعدادات" → opens this page modal.
  // - No sticky search bar (caused scroll-jank issues).
  // - All controls keep the same IDs the rest of the app uses, so the
  //   existing JS just works (toggles, save name button, theme picker, etc).
  function renderAllSettingsPage(container) {
    const escAttr = (s) => String(s ?? '').replace(/"/g, '&quot;').replace(/</g, '&lt;');
    const userName = window.App?.store?.get?.('userName') || '';

    container.innerHTML = `
      <div class="ded-all-settings">

        <!-- 🔍 Search (NOT sticky — sticky caused issues in PWA mode) -->
        <div class="settings-search-wrap-v2">
          <span class="settings-search-icon">🔍</span>
          <input type="text" id="allSettingsSearch" class="settings-search-input"
                 placeholder="ابحث في الإعدادات..." autocomplete="off">
          <button class="settings-search-clear" id="allSettingsSearchClear" hidden>✕</button>
        </div>
        <div class="settings-search-empty" id="allSettingsSearchEmpty" hidden>
          <div class="settings-search-empty-icon">🔍</div>
          <div class="settings-search-empty-text">ما لقينا إعدادات بهذا الاسم</div>
        </div>

        <section class="settings-section" data-keywords="حساب اسم بروفايل صورة">
          <h2 class="settings-section-title">👤 الحساب الشخصي</h2>
          <p class="settings-section-desc">اسمك وصورتك التي تظهر للآخرين</p>
          <div class="settings-section-body">
            <div class="settings-row" data-keywords="اسم name">
              <div class="settings-row-info">
                <div class="settings-row-title">الاسم</div>
                <div class="settings-row-desc">يظهر في صفحة الترحيب وللأصدقاء</div>
              </div>
              <div class="settings-row-control settings-row-control-full">
                <div class="form-row">
                  <input type="text" class="input" id="userNameInput" placeholder="اكتب اسمك..." maxlength="30" value="\${escAttr(userName)}">
                  <button class="btn-primary" id="saveUserNameBtn">احفظ</button>
                </div>
              </div>
            </div>
            <div class="settings-row" data-keywords="صورة عرض profile">
              <div class="settings-row-info">
                <div class="settings-row-title">صورة العرض</div>
                <div class="settings-row-desc">تظهر في الشات والبروفايل</div>
              </div>
              <div class="settings-row-control settings-row-control-full">
                <div class="settings-avatar-row">
                  <div class="conv-avatar my-profile-avatar" id="settingsProfileAvatar"
                       style="width:56px;height:56px;font-size:24px">👤</div>
                  <button class="btn-primary" id="changeProfilePicBtn">📷 تغيير الصورة</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <section class="settings-section" data-keywords="مظهر ثيم لون لغة">
          <h2 class="settings-section-title">🎨 المظهر</h2>
          <p class="settings-section-desc">الثيم واللغة</p>
          <div class="settings-section-body">
            <div class="settings-row" data-keywords="ثيم لون">
              <div class="settings-row-info">
                <div class="settings-row-title">الثيم</div>
                <div class="settings-row-desc">اختر مظهر التطبيق</div>
              </div>
              <div class="settings-row-control settings-row-control-full">
                <div class="themes-grid">
                  <button class="theme-btn" data-theme-id="default">🌑 افتراضي</button>
                  <button class="theme-btn" data-theme-id="light">☀️ فاتح</button>
                  <button class="theme-btn" data-theme-id="midnight">🌌 منتصف الليل</button>
                  <button class="theme-btn" data-theme-id="rose">🌸 وردي</button>
                  <button class="theme-btn" data-theme-id="tropical">🏝️ استوائي</button>
                  <button class="theme-btn" data-theme-id="sunset">🌅 غروب</button>
                  <button class="theme-btn" data-theme-id="oled">⚫ OLED</button>
                  <button class="theme-btn" data-theme-id="custom">🎨 مخصص</button>
                </div>
              </div>
            </div>
            <div class="settings-row" data-keywords="لغة language">
              <div class="settings-row-info">
                <div class="settings-row-title">اللغة</div>
                <div class="settings-row-desc">لغة واجهة التطبيق</div>
              </div>
              <div class="settings-row-control settings-row-control-full">
                <div class="lang-grid">
                  <button class="lang-btn active" data-lang="ar">
                    <span style="font-size:18px">🇸🇦</span><span>العربية</span>
                  </button>
                  <button class="lang-btn" data-lang="en">
                    <span style="font-size:18px">🇬🇧</span><span>English</span>
                  </button>
                </div>
              </div>
            </div>
        ...
        <section class="settings-section" data-keywords="تنبيهات إشعارات صوت">
          <h2 class="settings-section-title">🔔 التنبيهات</h2>
          <p class="settings-section-desc">كيف تصلك التنبيهات</p>
          <div class="settings-section-body">
            <div class="card">
              <div id="notifSettingsBody">
                <div class="notif-setting" data-keywords="toast إشعار">
                  <div class="notif-setting-info">
                    <div class="notif-setting-title">🔔 إشعار يظهر فوق (Toast)</div>
                    <div class="notif-setting-desc">إشعار حلو يطلع فوق الشاشة</div>
                  </div>
                  <div class="notif-toggle on" data-notif="toast" id="notifToggleToast"></div>
                </div>
                <div class="notif-setting" data-keywords="صوت sound">
                  <div class="notif-setting-info">
                    <div class="notif-setting-title">🎧 صوت تنبيه</div>
                    <div class="notif-setting-desc">صوت خفيف عند وصول رسالة</div>
                  </div>
                  <div class="notif-toggle on" data-notif="sound" id="notifToggleSound"></div>
                </div>
                <div class="notif-setting" data-keywords="شارة badge">
                  <div class="notif-setting-info">
                    <div class="notif-setting-title">🎯 شارة على التبويب</div>
                    <div class="notif-setting-desc">رقم الرسائل غير المقروءة</div>
                  </div>
                  <div class="notif-toggle on" data-notif="badge" id="notifToggleBadge"></div>
                </div>
              </div>
              <button class="btn-primary" id="testNotifBtn" style="width:100%;margin-top:14px">
                🎵 اختبار التنبيهات
              </button>
            </div>
          </div>
        ...
        <section class="settings-section" data-keywords="بيانات نسخ احتياطي تصدير export">
          <h2 class="settings-section-title">💾 البيانات والنسخ الاحتياطي</h2>
          <p class="settings-section-desc">صدّر تقاريرك واحتفظ بنسخة من بياناتك</p>
          <div class="settings-section-body">
            <div class="settings-row" data-keywords="تصدير csv pdf">
              <div class="settings-row-info">
                <div class="settings-row-title">تصدير تقرير الشهر</div>
                <div class="settings-row-desc">حمّل بياناتك بالصيغة المناسبة</div>
              </div>
              <div class="settings-row-control settings-row-control-full">
                <div class="export-row">
                  <button class="export-btn" data-export="csv">📊 CSV</button>
                  <button class="export-btn" data-export="txt">📄 نص</button>
                  <button class="export-btn" data-export="pdf">🖨 PDF</button>
                </div>
              </div>
            </div>
            <div class="settings-row" data-keywords="سنة بريد">
              <div class="settings-row-info">
                <div class="settings-row-title">تصدير متقدم</div>
                <div class="settings-row-desc">السنة كاملة أو إرسال للبريد</div>
              </div>
              <div class="settings-row-control settings-row-control-full">
                <div class="settings-stack-buttons">
                  <button class="export-btn" data-feature="export-year">📅 تصدير السنة كاملة (CSV)</button>
                  <button class="export-btn" data-feature="email-summary">📧 إرسال ملخص للبريد</button>
                </div>
              </div>
            </div>
            <div class="settings-row" data-keywords="نسخة backup restore">
              <div class="settings-row-info">
                <div class="settings-row-title">نسخة احتياطية</div>
                <div class="settings-row-desc">انقل بياناتك بين الأجهزة بسهولة</div>
              </div>
              <div class="settings-row-control settings-row-control-full">
                <div class="settings-stack-buttons">
                  <button class="export-btn" data-feature="backup">📤 نسخة احتياطية كاملة (JSON)</button>
                  <button class="export-btn" data-feature="restore">📥 استرجاع من ملف JSON</button>
                </div>
              </div>
            </div>
        ...
        <section class="settings-section" data-keywords="خصوصية أمان privacy">
          <h2 class="settings-section-title">🔒 الخصوصية والأمان</h2>
          <p class="settings-section-desc">بياناتك في يدك</p>
          <div class="settings-section-body">
            <div class="settings-row">
              <div class="settings-row-info">
                <div class="settings-row-title">تخزين البيانات</div>
                <div class="settings-row-desc">بياناتك محفوظة في جهازك فقط — لا تُرسل لأي خادم</div>
              </div>
              <div class="settings-row-control">
                <span class="settings-status-pill ok">🛡️ آمن</span>
              </div>
            </div>
            <div class="settings-row">
              <div class="settings-row-info">
                <div class="settings-row-title">العمل بدون إنترنت</div>
                <div class="settings-row-desc">التطبيق يشتغل أوفلاين بعد التحميل الأول</div>
              </div>
              <div class="settings-row-control">
                <span class="settings-status-pill ok">✓ مفعّل</span>
              </div>
            </div>
          </div>
        </section>

        <section class="settings-section settings-section-danger" data-keywords="مسح حذف خطر">
          <h2 class="settings-section-title">⚠️ منطقة الخطر</h2>
          <p class="settings-section-desc">إجراءات لا يمكن التراجع عنها</p>
          <div class="settings-section-body">
            <div class="settings-row">
              <div class="settings-row-info">
                <div class="settings-row-title">مسح كل البيانات</div>
                <div class="settings-row-desc">سيُحذف كل شيء نهائياً ولن تتمكن من الاسترجاع</div>
              </div>
              <div class="settings-row-control settings-row-control-full">
                <button class="btn-danger" id="clearBtn">🗑️ مسح كل البيانات</button>
              </div>
            </div>
          </div>
        </section>

        <section class="settings-section" data-keywords="عن التطبيق about">
          <h2 class="settings-section-title">ℹ️ عن التطبيق</h2>
          <div class="settings-section-body">
            <div class="card settings-about-card">
              <div class="settings-about-row"><b>تـدّبير PRO</b> — الإصدار 5.1.8</div>
              <div class="settings-about-row settings-about-muted">مخططك المالي الذكي باللغة العربية</div>
              <div class="settings-about-row settings-about-muted">بياناتك محفوظة في جهازك فقط 🔒</div>
              <div class="settings-about-divider"></div>
              <div class="settings-about-row settings-about-muted">
                ✨ يعمل أوفلاين • 🚀 PWA • 🛡️ خصوصية كاملة
              </div>
            </div>
          </div>
        </section>
    ...
    // Inject the userName value (template-literal escaping issue with backticks)
    const nameInput = container.querySelector('#userNameInput');
    if (nameInput) nameInput.value = userName;

    // ─── Live search ───
    const searchInput = container.querySelector('#allSettingsSearch');
    const clearBtn = container.querySelector('#allSettingsSearchClear');
    const emptyEl = container.querySelector('#allSettingsSearchEmpty');
    ...
    function applyFilter(query) {
      ...
      sections.forEach(section => {
        const t = section.querySelector('.settings-section-title')?.textContent || '';
        const d = section.querySelector('.settings-section-desc')?.textContent || '';
        const kw = section.dataset.keywords || '';
        const sectionMatch = !q || normalizeAr(`${t} ${d} ${kw}`).includes(q);
        const rows = section.querySelectorAll('.settings-row, .notif-setting');
        let rowsVisible = 0;
        rows.forEach(row => {
          const rt = row.querySelector('.settings-row-title, .notif-setting-title')?.textContent || '';
          const rd = row.querySelector('.settings-row-desc, .notif-setting-desc')?.textContent || '';
          const rkw = row.dataset.keywords || '';
          const rowMatch = !q || sectionMatch || normalizeAr(`${rt} ${rd} ${rkw}`).includes(q);
          row.style.display = rowMatch ? '' : 'none';
          if (rowMatch) rowsVisible++;
        });
        const showSection = !q || sectionMatch || rowsVisible > 0;
    ...
    const currentTheme = window.App?.store?.get?.('theme') || 'default';
    container.querySelectorAll('.theme-btn').forEach(btn => {
      ...
      btn.onclick = () => {
        ...
        if (window.ExtraThemes?.apply) window.ExtraThemes.apply(theme);
        else if (window.applyTheme) window.applyTheme(theme);
        container.querySelectorAll('.theme-btn').forEach(b =>
          b.classList.toggle('active', b === btn));
      };
    });

    // Notify any listeners that settings were freshly rendered
