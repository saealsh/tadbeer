...
var Controllers = (() => {
  ...
  function showTab(tab) {
    ...
    if (mainTab === 'home') {
      ...
      const currentSub = subTab || store.get('subTab') || 'achievements';

      // 🆕 SETTINGS REDESIGN (Apr 2026):
      // "⚙️ الإعدادات" was removed from the profile sub-nav. It now lives
      // as a dedicated full-screen page accessed from the sidebar (☰)
      // under "الحساب والإعدادات → كل الإعدادات".
      // - If someone tries to go to subTab=settings (from old code paths),
      //   redirect them to the dedicated all-settings page modal.
      // - Default sub-tab is now 'achievements' (was 'settings').

      if (currentSub === 'settings') {
        if (window.DedicatedPages?.open) {
          window.DedicatedPages.open('all-settings');
          store.set('subTab', 'achievements');
          if (profileSubNav) {
            profileSubNav.querySelectorAll('.sub-nav-btn').forEach(b =>
              b.classList.toggle('active', b.dataset.sub === 'achievements'));
          }
          return;
        }
        // Fallback if DedicatedPages not yet loaded
        actualTab = 'friends';
      } else if (currentSub === 'achievements') {
        // Same UX as before: open the achievements modal.
        if (window.DedicatedPages?.open) {
